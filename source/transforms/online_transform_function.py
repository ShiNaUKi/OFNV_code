...
class OnlineTransformFunction():
    def __init__(self, cont_indices, cat_indices, ord_indices, X=None, window_size=100):
        """
        ...
        p = len(cont_indices)               # 维度
        self.window_size = window_size
        self.window = np.array([[np.nan for x in range(p)] for y in range(self.window_size)]).astype(np.float64) #生成(windows x p)的nan矩阵
        self.update_pos = np.zeros(p).astype(np.int)
        if X is not None:
            self.partial_fit(X)
        
    def partial_fit(self, X_batch): #将X_batch里面的数据存储到windows里面去
        """
    ...
    def partial_evaluate_ord_latent_extendGC(self, X_batch):
        old = False
        ...
        ### 2.3 获取无序值的边界
        bounds = get_cat_bounds(X_cat, self.mu, self.cat_index_list, check=True, old=old)  # ***
